...
public class Chunk : MonoBehaviour
{
    ...
    {
        get { return MapManager.Ins.ChunkWidth; }
    }

    public static int height
    {
        get { return MapManager.Ins.ChunkHeight; }
    }
...
//        chunks.Add(this);
    ...
    {
        Random.InitState(MapManager.Ins.Seed);
//        Random.seed = World.currentWorld.seed;
    ...
    public virtual void CalculateMapFromScratch()
    {
        ...
        for (int x = 0; x < MapManager.Ins.ChunkWidth; x++)
        {
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int z = 0; z < width; z++)
                {
                    ...
                    // Top wall
                    if (IsTransparent(x, y + 1, z))
                        BuildFace(brick, new Vector3(x, y + 1, z), Vector3.forward, Vector3.right, true, verts, uvs,
                            tris);
    ...
    {
        for (int i = 0; i < MapManager.Chunks.Count; i++)
        {
            Vector3 cpos = MapManager.Chunks[i].transform.position;

            if (pos.x < cpos.x || pos.z < cpos.z || pos.x >= cpos.x + width ||
                pos.z >= cpos.z + width) continue;
            return MapManager.Chunks[i];
        }
        return null;
    }
